...
resource "aws_instance" "Nginx1_ec2" {
  ...
  tags = {
    Name = "nginx1_instance"
  }
  key_name = aws_key_pair.my_key_pair.key_name
    # Remote-exec provisioner to install Nginx on EC2 instance
   provisioner "file" {
...
output "EC2_Nginx1_id" {
  value = aws_instance.Nginx1_ec2.id
}
output "Nginx1_public_ip" {
    value = aws_instance.Nginx1_ec2.public_ip
  
}
output "Nginx1_private_ip" {
    value = aws_instance.Nginx1_ec2.private_ip
